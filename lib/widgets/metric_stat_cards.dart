...
// ============================================================
// Ekran suresi karti (grafiklerde her zaman gosterilir)
// ============================================================
class ScreenTimeChartCard extends StatefulWidget {
  final Map<DateTime, int> minutes; // gun -> dakika (TUM veri)
  final DateTime today;
  const ScreenTimeChartCard({
    super.key,
  ...
  @override
  State<ScreenTimeChartCard> createState() => _ScreenTimeChartCardState();
}

class _ScreenTimeChartCardState extends State<ScreenTimeChartCard> {
  int _days = 30;

  @override
  Widget build(BuildContext context) {
    final from = widget.today.subtract(Duration(days: _days - 1));
    // Dakikayi saate cevir (grafik daha okunakli).
    final series = <DateTime, double>{
      for (final e in widget.minutes.entries) e.key: e.value / 60.0,
    };
    final window = _window(series, from);
    final values = window.values.toList();
    final avg =
        values.isEmpty ? 0.0 : values.reduce((a, b) => a + b) / values.length;
    ...
    return _cardShell(
      ...
      trailing:
          _RangeSelector(days: _days, onChanged: (d) => setState(() => _days = d)),
    );
  }
}
